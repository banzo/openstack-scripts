...
DEST_IP=$(openstack port show dest_vm_port_t4 -f value -c fixed_ips|grep "ip_address='[0-9]*\."|cut -d"'" -f2)
neutron flow-classifier-create \
    --ethertype IPv4 \
    --source-ip-prefix ${SOURCE_IP}/32 \
    --destination-ip-prefix ${DEST_IP}/32 \
    --protocol tcp \
    --destination-port 80:80 \
    --logical-source-port source_vm_port_t4 \
    FC_http_t4
...
# Create the port pairs for all 3 VMs
neutron port-pair-create --ingress=p1in_t4 --egress=p1out_t4 PP1_t4
neutron port-pair-create --ingress=p2in_t4 --egress=p2out_t4 PP2_t4
neutron port-pair-create --ingress=p3in_t4 --egress=p3out_t4 PP3_t4

# And the port pair groups
neutron port-pair-group-create --port-pair PP1_t4 --port-pair PP2_t4 PG1_t4
neutron port-pair-group-create --port-pair PP3_t4 PG2_t4

# The complete chain
neutron port-chain-create --port-pair-group PG1_t4 --port-pair-group PG2_t4 --flow-classifier FC_udp_t4 --flow-classifier FC_http_t4 PCt41

sleep 5

# Start a basic demo web server
ssh -o StrictHostKeyChecking=no cirros@${DEST_IP} 'while true; do echo -e "HTTP/1.0 200 OK\r\n\r\nWelcome to $(hostname) t4" | sudo nc -l -p 80 ; done&'

# On service VMs, enable eth1 interface and add static routing
for sfc_port in p1in_t4 p2in_t4 p3in_t4
do
    ssh -o StrictHostKeyChecking=no -T cirros@$(openstack port show ${sfc_port} -f value -c fixed_ips|grep "ip_address='[0-9]*\."|cut -d"'" -f2) <<EOF
sudo sh -c 'echo "auto eth1" >> /etc/network/interfaces'
sudo sh -c 'echo "iface eth1 inet dhcp" >> /etc/network/interfaces'
